...
% Imports logfile into table and processes the log messages

    try
        % read the TAB delimited logfile into a table
    ...
    % Extract node ID from str
    a = char(tbl.IDStr);
    tbl.ID = str2double(cellstr(a(:, 4:end)));
    clear a

    % Check if Output is a message intended for performance evaluation
    tbl.EVAL_MSG = logical(cell2mat(cellfun(@(x) ~isempty(x), ...
